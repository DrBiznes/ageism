...
import { FACE_GENDER, ROLE_FACES, type FaceId } from "./faceAvatarOptions";
...
export function AvatarPicker({ role, selected, onSelect }: AvatarPickerProps) {
  ...
  const groupedFaces = [
    { label: "Male options", faces: faces.filter((faceId) => FACE_GENDER[faceId] === "male") },
    { label: "Female options", faces: faces.filter((faceId) => FACE_GENDER[faceId] === "female") },
  ];
      ...
      {groupedFaces.map((group) => (
        <div key={group.label}>
          ...
          <div className="grid grid-cols-4 gap-3">
            {group.faces.map((faceId) => (
              <button
                key={faceId}
                type="button"
                onClick={() => onSelect(faceId)}
                className="relative overflow-hidden transition-all"
                aria-label={`Choose ${group.label.toLowerCase().replace(" options", "")} avatar`}
                style={{
